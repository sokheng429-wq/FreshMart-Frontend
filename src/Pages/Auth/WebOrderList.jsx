...
import { adminWebOrderAPI, adminCustomerAPI, adminProductAPI } from '../../api/api'
import mobileIcon from '../../assets/icon/3dicons-mobile-dynamic-color.png'
import './ProductsHub.css'

// Requested Columns for Web Order
const ALL_COLUMNS = [
  { key: 'code', label: { en: 'Code', kh: 'លេខកូដ' }, always: true },
  { key: 'orderCode', label: { en: 'Order Code', kh: 'កូដបញ្ជាទិញ' }, always: true },
  ...
  { key: 'markup', label: { en: 'Markup', kh: 'ប្រាក់បន្ថែម ($)' } },
...
const STATUS_CONFIG = {
  ...
  READY_TO_SHIP: { labelEn: 'Ready to Ship', color: 'bg-purple-500/20 text-purple-400 border-purple-500/30' },
  PROCESSING: { labelEn: 'Processing', color: 'bg-blue-500/20 text-blue-400 border-blue-500/30' },
  PENDING: { labelEn: 'Pending', color: 'bg-amber-500/20 text-amber-400 border-amber-500/30' },
...
export default function WebOrderList() {
  ...
  const [visibleCols, setVisibleCols] = useState(() => {
    try {
      const saved = localStorage.getItem('bg_web_order_cols_v2')
  ...
  const [outletInput, setOutletInput] = useState('Online Store')
  const [referenceInput, setReferenceInput] = useState('')
  const [usernameInput, setUsernameInput] = useState('online_user')
  const [markupInput, setMarkupInput] = useState(0)
  const [barcodeHintInput, setBarcodeHintInput] = useState('')
  const [showProductPicker, setShowProductPicker] = useState(false)

  const [formItems, setFormItems] = useState([
    { id: 1, productId: 18, productCode: 'PRD-0006', description: 'Monster Energy - Zero Sugar Blue', qty: 2, price: 3.45, total: 6.9 },
  ])
  ...
  const loadOrders = useCallback(async () => {
    setLoading(true)
    try {
      const res = await adminWebOrderAPI.getAll({
        search: searchText,
        ...
        setOrders([
          {
            id: 1,
            code: 'WEB-0001',
            orderCode: 'WEB-202609-0891',
            deliveryDate: '2026-09-03T18:00:00',
            salesperson: 'Admin',
            customerName: 'Chenda Pich',
            phone: '012 888 777',
            grandTotal: 45.5,
            balance: 0,
            status: 'PROCESSING',
            reference: 'REF-WEB-01',
            username: 'chenda_p',
            markup: 0,
            outlet: 'Online Store',
          },
          {
            id: 2,
            code: 'WEB-0002',
            orderCode: 'WEB-202609-0892',
            deliveryDate: '2026-09-04T10:30:00',
            salesperson: 'Sok Heng',
            customerName: 'Dara Sam',
            phone: '097 555 1234',
            grandTotal: 112.0,
            balance: 112.0,
            status: 'READY_TO_SHIP',
            reference: 'REF-WEB-02',
            username: 'dara_s',
            markup: 5.0,
            outlet: 'Online Store',
          },
  ...
  const handleSelectProduct = (p) => {
    ...
    const code = p.code || ''
    setFormItems((prev) => [...prev, { id: Date.now(), productId: p.id, productCode: code, description: pName, qty: 1, price, total: price }])
    setBarcodeHintInput('')
    setShowProductPicker(false)
    addNotification?.({ type: 'success', message: `${pName} added to Web Order!` })
  }

  const grandTotal = useMemo(() => {
    return formItems.reduce((sum, it) => sum + (it.qty || 1) * (it.price || 0), 0) + Number(markupInput || 0)
  }, [formItems, markupInput])

  const handleOpenCreate = async () => {
    try {
      const res = await adminWebOrderAPI.getNextCode()
      setFormCode(res?.data?.code || res?.code || `WEB-${Math.floor(1000 + Math.random() * 9000)}`)
    } catch {
      setFormCode(`WEB-${Math.floor(1000 + Math.random() * 9000)}`)
    }
    const now = new Date()
    setDeliveryDateInput(new Date(now.getTime() + 24 * 60 * 60 * 1000).toISOString().slice(0, 16))
  ...
  const handleSaveOrder = async () => {
    ...
    try {
      await adminWebOrderAPI.create(newOrder)
      addNotification?.({ type: 'success', message: `Web Order ${formCode} registered!` })
      setShowCreateModal(false)
      loadOrders()
    } catch {
      setOrders((prev) => [{ id: Date.now(), ...newOrder }, ...prev])
      setShowCreateModal(false)
      addNotification?.({ type: 'success', message: `Web Order ${formCode} saved!` })
    } finally {
  ...
  return (
    <div className="space-y-6 text-slate-100 pb-12" style={{ fontFamily: 'Montserrat, sans-serif' }}>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-400">
            ...
            <span>/</span>
            <span className="text-amber-400 font-bold">Web Order</span>
          </div>

          <div className="flex items-center gap-3 mt-2">
            <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-amber-500/20 p-2 ring-1 ring-amber-500/30">
              <img src={mobileIcon} alt="" className="h-7 w-7 object-contain drop-shadow" />
            </span>
            <div>
              <h1 className="text-2xl font-black text-white tracking-tight">
                {lang === 'en' ? 'Web Order Management' : 'ការគ្រប់គ្រងការបញ្ជាទិញលើគេហទំព័រ'}
              </h1>
              <p className="text-xs text-slate-400">
                Show information of web order list. Ex(Code, Date, Customer...)
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Search web order by any condition. Ex(Any, Code, Customer...)
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-12">
          <div className="sm:col-span-3">
            ...
            <select
              ...
              className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-xs font-semibold text-white outline-none focus:border-amber-400"
            >
              <option value="any">Any</option>
              <option value="code">Code / Order Code</option>
          ...
          <div className="sm:col-span-7">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-700 bg-slate-950 py-2 pl-9 pr-3 text-xs font-semibold text-white placeholder-slate-500 outline-none focus:border-amber-400"
              />
            </div>
          </div>
          ...
          <div className="sm:col-span-2 flex items-end">
            <button
              type="button"
              onClick={loadOrders}
              className="w-full rounded-xl bg-amber-600 px-4 py-2 text-xs font-bold text-white shadow-md shadow-amber-600/30 transition hover:bg-amber-500"
            >
              Search
            </button>
          </div>
      ...
      {/* 3. WEB ORDER LIST TABLE */}
      <div className="rounded-3xl border border-slate-800 bg-slate-900/80 shadow-2xl overflow-hidden">
        <div className="p-4 border-b border-slate-800 flex justify-between items-center">
          <div>
            <h3 className="text-sm font-bold text-white uppercase tracking-wider">Web Order List</h3>
            <p className="text-xs text-slate-400">Show information of web order list. Ex(Code, Date, Customer...)</p>
          </div>
        </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-slate-800 bg-slate-950/80 text-slate-400 font-bold uppercase tracking-wider text-[10px]">
                ...
                {visibleCols.has('orderCode') && <th className="py-3.5 px-4">Order Code</th>}
                ...
                {visibleCols.has('markup') && <th className="py-3.5 px-4 text-right">Markup</th>}
                {visibleCols.has('outlet') && <th className="py-3.5 px-4">Outlet</th>}
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/60 font-medium">
              {loading ? (
                <tr><td colSpan={13} className="py-12 text-center text-slate-400">Loading Web Orders...</td></tr>
              ) : orders.length === 0 ? (
                <tr><td colSpan={13} className="py-12 text-center text-slate-400">No Web Orders found.</td></tr>
              ) : (
                orders.map((o) => {
                  const statusInfo = STATUS_CONFIG[o.status] || STATUS_CONFIG.PENDING
                  return (
                    <tr key={o.id} className="hover:bg-slate-800/40 transition">
                      {visibleCols.has('code') && <td className="py-3 px-4 font-mono font-bold text-amber-400">{o.code}</td>}
                      {visibleCols.has('orderCode') && <td className="py-3 px-4 font-mono text-slate-300">{o.orderCode || o.code}</td>}
                      ...
                      {visibleCols.has('phone') && <td className="py-3 px-4 font-mono text-slate-400">{o.phone || '---'}</td>}
                      ...
                      {visibleCols.has('username') && <td className="py-3 px-4 text-slate-400 text-[11px]">{o.username || 'online'}</td>}
                      {visibleCols.has('markup') && <td className="py-3 px-4 text-right font-mono text-slate-300">{formatCurrency(o.markup)}</td>}
                      {visibleCols.has('outlet') && <td className="py-3 px-4 text-slate-400 text-[11px]">{o.outlet || 'Online Store'}</td>}
                    </tr>
                  )
                })
              )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg rounded-3xl border border-slate-700 bg-slate-900 p-6 shadow-2xl space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 max-h-[320px] overflow-y-auto">
              {ALL_COLUMNS.map((col) => (
                <label key={col.key} className="flex items-center gap-2 p-2 rounded-xl bg-slate-950/60 border border-slate-800 text-xs text-slate-300 hover:border-amber-500/30 cursor-pointer transition">
                  <input
                    type="checkbox"
            ...
            <div className="flex justify-end gap-2 pt-3 border-t border-slate-800">
              ...
              <button
                onClick={() => {
                  ...
                  localStorage.setItem('bg_web_order_cols_v2', JSON.stringify(Array.from(colDraft)))
                  setShowColModal(false)
                }}
                className="px-4 py-1.5 rounded-xl bg-amber-600 text-xs font-bold text-white hover:bg-amber-500"
              >
                Apply
              </button>
            </div>
      ...
      {/* 5. CREATE MODAL WITH LIVE PRODUCT AUTOCOMPLETE */}
      {showCreateModal && (
          ...
          <div className="w-full max-w-4xl rounded-3xl border border-slate-700 bg-[#0f172a] p-6 space-y-5 max-h-[92vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-3">
                <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-500/20 text-amber-400">🌐</span>
                <div>
                  <h3 className="text-lg font-black text-white">Create Web Order</h3>
                  <p className="text-xs text-slate-400">Enter customer details and pick live products</p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-slate-950/60 p-4 rounded-2xl border border-slate-800">
              ...
              <div>
                ...
                <input type="text" value={phoneInput} onChange={(e) => setPhoneInput(e.target.value)} placeholder="012..." className="w-full rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-xs text-white font-mono" />
              </div>
              <div>
                <label className="block text-[11px] font-bold text-slate-300 mb-1">Delivery Date</label>
                <input type="datetime-local" value={deliveryDateInput} onChange={(e) => setDeliveryDateInput(e.target.value)} className="w-full rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-xs text-white font-mono" />
              </div>
              <div>
                <label className="block text-[11px] font-bold text-slate-300 mb-1">Salesperson</label>
                <input type="text" value={salespersonInput} onChange={(e) => setSalespersonInput(e.target.value)} className="w-full rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-xs text-white" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-300 mb-1">Order Code</label>
                <input type="text" value={formCode} onChange={(e) => setFormCode(e.target.value)} className="w-full rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-xs font-mono font-bold text-amber-400" />
              </div>
            </div>
            ...
            <div className="relative">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-[10px] text-amber-400 font-semibold">{productCatalog.length} live products loaded</span>
              </div>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-dashed border-amber-500/50 bg-slate-950/90 py-2.5 pl-9 pr-28 text-xs font-semibold text-white placeholder-slate-500 outline-none focus:border-amber-400"
                />
                <button
                  type="button"
                  onClick={() => setShowProductPicker(!showProductPicker)}
                  className="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg bg-amber-600/30 border border-amber-500/40 px-2.5 py-1 text-[10px] font-bold text-amber-300 hover:bg-amber-600 hover:text-white transition"
                >
              ...
              {showProductPicker && (
                <div className="absolute left-0 right-0 top-full mt-1.5 z-40 max-h-60 overflow-y-auto rounded-2xl border border-amber-500/30 bg-slate-900 shadow-2xl p-2 space-y-1">
                  {filteredProducts.map((p) => (
                    <div
                      key={p.id}
                      onClick={() => handleSelectProduct(p)}
                      className="flex items-center justify-between p-2 rounded-xl hover:bg-amber-500/10 cursor-pointer border border-transparent hover:border-amber-500/30"
                    >
                      <div>
                        <p className="text-xs font-bold text-white">{p.name || p.nameKh} <span className="font-mono text-amber-400">({p.code})</span></p>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] bg-amber-600 text-white px-2 py-0.5 rounded font-bold">+ Add</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="flex items-center justify-between border-t border-slate-800 pt-3">
              <span className="font-mono text-base font-bold text-emerald-400">Grand Total: {formatCurrency(grandTotal)}</span>
              <div className="flex gap-2">
                ...
                <button onClick={handleSaveOrder} disabled={savingOrder} className="px-5 py-1.5 rounded-xl bg-amber-600 text-xs font-bold text-white hover:bg-amber-500">
                  Submit Web Order
                </button>
              </div>
            </div>
